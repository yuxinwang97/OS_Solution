...
void stopbutton(){
	...
	//printf("%s",str);
	if (write(1, str, strlen(str)) < 0)
		print_err("fail to write shutdown");
	if (log_flag) fputs(str,log_name); //TODO error handling
	exit(0);
}
...
void setup_socket() {
	...
	server = gethostbyname(which_host);
	//printf("debugging: host:%s, socketfd:%i, port:%i\n"which_host, socket_fd, );
    if (server == NULL)  { 
	...
	close(0);
    if ( dup(socketfd) == -1) print_err("redirect stdin to socket file descripter\n");
    close(1);
    if ( dup(socketfd) == -1) print_err("redirect stdin to socket file descripter\n");
    close(socketfd);
    char id_buffer[50];
	sprintf(id_buffer, "ID=%d\n", myid);
    if (write(1, id_buffer, strlen(id_buffer)) < 0)
		print_err("fail to write id");
